...
public:
    ...
    BAdmm()
    {
        ...
        trajectory_pub = nh.advertise<agv_admm::FloatArray>("/b/trajectory", 1);
        local_copies_pub = nh.advertise<agv_admm::FloatArray>("/b/local_copies", 1);
        lambda_multipliers_pub = nh.advertise<agv_admm::FloatArray>("/b/lambda_multipliers", 1);
        vel_ref_pub = nh.advertise<geometry_msgs::Pose2D>("/b/velocity_reference", 1);
        path_pub = nh.advertise<nav_msgs::Path>("/b/horizon_path", 1);
        ocpX_time_pub = nh.advertise<std_msgs::Float64>("/b/ocpX_time", 1);
        ocpZ_time_pub = nh.advertise<std_msgs::Float64>("/b/ocpZ_time", 1);
        //cbf_time_pub = nh.advertise<std_msgs::Float64>("cbf_time", 1);

        guessedi_path_pub = nh.advertise<nav_msgs::Path>("/b/guessedi_horizon_path", 1);
        guessedj_path_pub = nh.advertise<nav_msgs::Path>("/b/guessedj_horizon_path", 1);

        reference_pose_sub = nh.subscribe("/b/reference_pose", 1, &BAdmm::reference_callback, this);
        counter_reset_sub = nh.subscribe("/counter_restart", 1, &BAdmm::reset_callback, this);
        pose_sub = nh.subscribe("/b/pose", 1, &BAdmm::pose_callback, this);

        trajectory_sub = nh.subscribe("/u/trajectory", 1, &BAdmm::trajectory_callback, this);
        local_copies_sub = nh.subscribe("/u/local_copies", 1, &BAdmm::local_copies_callback, this);
        lambda_multipliers_sub = nh.subscribe("/u/lambda_multipliers", 1, &BAdmm::lambda_multipliers_callback, this);
        ...
        ocpX_function = casadi::Function::load(FUNCTIONS_DIR + "/b_ocpX.casadi");
        ocpZ_function = casadi::Function::load(FUNCTIONS_DIR + "/b_ocpZ.casadi");
...
int main(int argc, char *argv[])
{
    ...
    bAdmm.ocpX_function.release(bAdmm.mem_ocpX);
    bAdmm.ocpZ_function.release(bAdmm.mem_ocpZ);
    //bAdmm.ocpC_function.release(bAdmm.mem_ocpC);
