...
public:
    ...
    DynamicModel()
    {
        ...
        inertial_pose_pub = n.advertise<geometry_msgs::Pose2D>("/u/pose", 1);
        odom_pub = n.advertise<nav_msgs::Odometry>("/u/odom", 1);

        vel_sub = n.subscribe("/u/velocity_reference", 1, &DynamicModel::vel_callback, this);
    ...
    void vel_callback(const geometry_msgs::Pose2D::ConstPtr& _vel)
    {
        ...
        w = _vel->theta; //rotational moment in Nm
    }

    void time_step()
    {
        
        xs_dot << v*cos(psi), v*sin(psi), w;
...
{
    ros::init(argc, argv, "u_kinematics");
